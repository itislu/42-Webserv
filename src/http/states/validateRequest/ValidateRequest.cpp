 ...
 * 5. Normalize again, disallowing going out of root.
 * 6. Combine with root.
 */
  ...
  NormalizationMode mode)
{
  ...
  while (std::getline(stream, token, '/').good()) {
  ...
  ft::optional<std::string> final = "/";
  for (std::size_t i = 0; i < segments.size(); ++i) {
    *final += segments[i];
    if (i + 1 < segments.size() || ft::ends_with(path, '/')) {
      *final += "/";
    }
  }
  return final;
