...
const long ParseUri::defaultMaxUriLength;
long ParseUri::_maxUriLength = defaultMaxUriLength;
...
void ParseUri::setMaxUriLength(long length)
{
...
bool ParseUri::_requestTargetTooLong()
{
  if (_buffReader.getPosInBuff() + 1 > _maxUriLength) {
    _log.error() << "Uri too long\n";
